create table DmKhoa
(
MaKhoa nvarchar(50) primary key
,TenKhoa nvarchar(50) null
)
create table DmNganh
(
MaNganh nvarchar(50) primary key
,TenNganh nvarchar(50) null
,MaKhoa nvarchar(50) foreign key references DmKhoa(MaKhoa) on update cascade on delete cascade
)
create table DmLop
(
MaLop nvarchar(50) primary key
,TenLop nvarchar(50) null
,MaNganh nvarchar(50) foreign key references DmNganh(MaNganh) on update cascade on delete cascade
,KhoaHoc nvarchar(50) null
,HeDaoTao nvarchar(50) null
,NamNhapHoc int null check (NamNhapHoc>=2022)
)
create table DmHocPhan
(
MaHP nvarchar(50) primary key
,TenHP nvarchar(50) null
,SoTinChi int null check (SoTinChi>=2 and SoTinChi<=10)
,MaNganh nvarchar(50) foreign key references DmNganh(MaNganh) on update cascade on delete cascade
,HocKy nvarchar(50) null check (HocKy in('1','2','3','4','5','6','7','8'))
)
create table SinhVien
(
MaSinhVien nvarchar(50) primary key
,HoTen nvarchar(50) not null
,MaLop nvarchar(50) foreign key references DmLop(MaLop) on update cascade on delete cascade
,GioiTinh bit not null
,NgaySinh date not null
,DiaChi nvarchar (max) default N'Chưa xác định'
)
create table DiemHP
(
MaSinhVien nvarchar(50)
,MaHP nvarchar(50)
,DiemHP float null check (DiemHP Between 0 and 10)
,constraint PK_DiemHP primary key (MaSinhVien,MaHP)
,constraint FK_SV foreign key(MaSinhVien) references SinhVien(MaSinhVien)
,constraint FK_HP foreign key(MaHP) references DmHocPhan(MaHP)
)

